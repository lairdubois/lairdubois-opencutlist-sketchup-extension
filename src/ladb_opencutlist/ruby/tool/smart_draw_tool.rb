module Ladb::OpenCutList
  ...
  class SmartDrawTool < SmartTool
    ...
    def onCancel(reason, view)
      ...
      @action_handler.onCancel(reason, view) unless @action_handler.nil?
      return
    ...
    def onMouseMove(flags, x, y, view)
      ...
      if _picked_pushpull_point?
        ...
        unless view.inference_locked?
          ...
          k_line.start.copy!(@picked_pushpull_ip.position)
        ...
        if is_action_draw_rectangle?
          ...
          @direction = nil unless @direction.valid?

        elsif is_action_draw_polygon?
        end

        @snap_ip.copy!(@mouse_ip) unless @snap_ip.valid?

        if _fetch_option_rectangle_centered || is_action_draw_circle?
        ...
        pts = _get_local_section_points

        section_offset = _fetch_option_section_offset
        if section_offset != 0

          o_pts = Fiddle::Clippy.rpath_to_points(Fiddle::Clippy.inflate_paths(
            paths: [ Fiddle::Clippy.points_to_rpath(pts) ],
            delta: section_offset,
            join_type: Fiddle::Clippy::JOIN_TYPE_MITER
          ).first)

          k_segments = Kuix::Segments.new
          k_segments.add_segments(_points_to_segments(pts))
        ...
        k_segments = Kuix::Segments.new
        k_segments.add_segments(_points_to_segments(o_pts))
        k_segments.line_width = @locked_normal ? 3 : 1.5
        k_segments.line_stipple = Kuix::LINE_STIPPLE_SHORT_DASHES if fetch_action_option_enabled(fetch_action, ACTION_OPTION_OPTIONS, ACTION_OPTION_OPTIONS_CONSTRUCTION)
        k_segments.color = _get_normal_color
        k_segments.transformation = t
        @space.append(k_segments)
        ...
        if is_action_draw_rectangle?
          ...
          @floating_panel.append(k_label_r)
    ...
    def onLButtonUp(flags, x, y, view)
      ...
      if !_picked_section_first_point?
        ...
        if @snap_ip.position.distance(@picked_section_first_ip.position) > view.pixels_to_model(50, @snap_ip.position)  # Drag handled only if distance is > 50px
          if _valid_rectangle?
    ...
    def onKeyUp(key, repeat, flags, view)
      ...
      if key == VK_ALT
        store_action_option_value(ACTION_DRAW_RECTANGLE, ACTION_OPTION_OPTIONS, ACTION_OPTION_OPTIONS_RECTANGLE_CENTRED, !_fetch_option_rectangle_centered, true) if is_action_draw_rectangle? && _picked_section_first_point? && !_picked_section_last_point?
        store_action_option_value(ACTION_DRAW_RECTANGLE, ACTION_OPTION_OPTIONS, ACTION_OPTION_OPTIONS_BOX_CENTRED, !_fetch_option_box_centered, true) if _picked_section_first_point? && _picked_section_last_point? && !_picked_pushpull_point?
        _refresh
      end
      ...
      if _picked_pushpull_point?
        ...
        if d1 || d2
          ...
          if radius == 0
            ...
            length = radius / 2 if _fetch_option_rectangle_centered
          end
          ...
          @picked_section_last_ip = Sketchup::InputPoint.new(Geom::Point3d.new(p1.x + radius, p1.y + width, p1.z).transform(t))
    ...
    def _fetch_option_segment_count
      fetch_action_option_value(ACTION_DRAW_CIRCLE, ACTION_OPTION_SEGMENTS, ACTION_OPTION_SEGMENTS_SEGMENT_COUNT).to_i
    end

    def _fetch_option_rectangle_centered
      fetch_action_option_enabled(ACTION_DRAW_RECTANGLE, ACTION_OPTION_OPTIONS, ACTION_OPTION_OPTIONS_RECTANGLE_CENTRED)
    end

    def _fetch_option_box_centered
      fetch_action_option_enabled(fetch_action, ACTION_OPTION_OPTIONS, ACTION_OPTION_OPTIONS_BOX_CENTRED)
    end
    ...
    def _get_local_section_points
      ...
      if is_action_draw_rectangle?
        ...
        segment_count = [ fetch_action_option_value(ACTION_DRAW_CIRCLE, ACTION_OPTION_SEGMENTS, ACTION_OPTION_SEGMENTS_SEGMENT_COUNT).to_i, 3 ].max
        ...
        return Array.new(segment_count) { |i| Geometrix::CircleFinder.circle_point_at_angle(circle_def, start_angle + i * unit_angle) }
    ...
    def _create_entity
      ...
      if fetch_action_option_enabled(fetch_action, ACTION_OPTION_OPTIONS, ACTION_OPTION_OPTIONS_CONSTRUCTION)
        ...
        segments = _points_to_segments(_get_local_section_points, true, false)
        segments.each do |segment|
          group.entities.add_cline(*segment)
        end
        ...
        if is_action_draw_circle?
          edge = definition.entities.add_circle(p1, Z_AXIS, p1.distance(p2), _fetch_option_segment_count).first
          edge.find_faces
          face = edge.faces.first
        else
          face = definition.entities.add_face(_get_local_section_points)
        end
  ...
  class DrawRectangleActionHandler < DrawActionHandler
    ...
    def _pick_move_point(flags, x, y, view)
      ...
      k_box.bounds.size.set!(o_bounds.width, o_bounds.height, o_bounds.depth)
      k_box.line_width = 1.5
      k_box.line_stipple = Kuix::LINE_STIPPLE_SHORT_DASHES if _fetch_option_construction
      k_box.color = _get_normal_color
      k_box.transformation = t * mt
      @tool.append_3d(k_box)
