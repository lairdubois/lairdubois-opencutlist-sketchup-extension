      ...
      if @path
        ...
        begin
          ...
          rows.each do |row|
            ...
            # Errors
            if part[:name].nil?
              part[:errors] << 'tab.importers.csv.error.invalid_name'
            end
            if part[:length].nil?
              part[:errors] << 'tab.importers.csv.error.invalid_length'
            end
            if part[:width].nil?
              part[:errors] << 'tab.importers.csv.error.invalid_width'
            end
            if part[:thickness].nil?
              part[:errors] << 'tab.importers.csv.error.invalid_thickness'
            end

            # Warnings
            if part[:count].nil?
              part[:warnings] << 'tab.importers.csv.warning.invalid_count'
            end
            if part[:material].nil?
              part[:warnings] << 'tab.importers.csv.warning.invalid_material'
            end
          ...
          if response[:importable_part_count] == 0
            if response[:parts].length == 0
              response[:errors] << 'tab.importers.csv.error.empty_file'
            else
              response[:errors] << 'tab.importers.csv.error.no_importable_part'
            end
          end
