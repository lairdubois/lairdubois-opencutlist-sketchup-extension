module Ladb::OpenCutList
  ...
  class ImportersCsvOpenWorker
    ...
    def run
      ...
      path = @path.is_a?(String) ? @path : UI.openpanel(PLUGIN.get_i18n_string('tab.importers.csv.load.title'), '', "CSV|*.csv|TSV|*.tsv||")
      if path
        ...
        if extname.nil? || extname.downcase != '.csv' && extname.downcase != '.tsv'
          response[:errors] << [ 'tab.importers.default.error.bad_extension', { :filename => filename, :extensions => 'CSV, TSV' } ]
          return response
        end
