    ...
    def run
      ...
      ld.each do |layer_def|
        ...
        polygons = layer_def[:paths].map { |path|
          next if Clippy.get_path_area(path).abs < MINIMAL_PATH_AREA # Ignore "artifact" paths generated by successive transformation / union / differences
          DrawingProjectionPolygonDef.new(Clippy.path_to_points(path, z_max - layer_def[:depth]), Clippy.is_path_positive?(path))
        }.compact
